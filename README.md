...
<!--
  Coverage badges — generated by CI on every push to main and
  pushed to the `badges` orphan branch. The SVGs are committed
  there (no third-party rendering service); the URLs below
  point at GitHub's raw content host.

  Replace `OWNER/REPO` with your actual GitHub `<owner>/<repo>`
  before the badges resolve. They'll be 404 until the badges
  branch exists, which happens automatically after the first
