...
<!--
  ...
  remote-hit pulses. The whole pitch of mixchamb lands faster from
...
| **Ecto + Postgres** | Anonymous users + chambers schemas; admin force-delete; sweepers aging out idle rows | `Mixchamb.Accounts`, `Mixchamb.Chambers` |
...
fly apps create mixchamb

# 3. Provision Postgres + attach it. Fly creates DATABASE_URL
#    automatically and injects it as a secret on the app.
fly mpg create --name mixchamb-db --org personal --plan basic
fly mpg attach <db-name> --app mixchamb

# 4. Set the rest of the secrets.
fly secrets set --app mixchamb \
...
`primary_region`. Visit `https://mixchamb.fly.dev`.
...
`mixchamb@<ipv6>`. `dns_cluster` polls `mixchamb.internal` (Fly's
...
mixchamb@fdaa:0:abcd::1                    self    132    24.3 MB
mixchamb@fdaa:0:abcd::2                    14s     128    23.7 MB
```
